...
# test_labels = np.vstack((np.zeros((50, 1)),np.ones((50, 1)),2*np.ones((50, 1)), 3*np.ones((50, 1)),4*np.ones((50, 1)),5*np.ones((50, 1))))
...
test_z = np.loadtxt('C:/Users/THUNDEROBOT/Desktop/ljs24.4.23/dagmm-master-myhmm1221/test_z.txt', dtype=str, delimiter=' ').astype('float32')
# row_to_delete = np.concatenate([np.arange(700,1000),np.arange(1100,1400),np.arange(1500,1800)])
# row_to_delete = np.concatenate([np.arange(100,200),np.arange(300,400),np.arange(500,600)])
...
# t-SNE
np.random.seed(0)
tsne = TSNE(n_components=2,perplexity = 30, n_iter=1000, init='random', random_state=0, learning_rate= 5)
...
plt.scatter(embedded_data[np.squeeze(test_labels ==0), 0], embedded_data[np.squeeze(test_labels ==0), 1], label=u'工况1正常样本', c = 'navy', marker='o')
plt.scatter(embedded_data[np.squeeze(test_labels ==1), 0], embedded_data[np.squeeze(test_labels ==1), 1], label=u'工况2正常样本', c = 'blue', marker='o')
plt.scatter(embedded_data[np.squeeze(test_labels ==2), 0], embedded_data[np.squeeze(test_labels ==2), 1], label=u'工况3正常样本', c = 'cornflowerblue', marker='o')
plt.scatter(embedded_data[np.squeeze(test_labels ==3), 0], embedded_data[np.squeeze(test_labels ==3), 1], label=u'工况1异常样本', c = 'red', marker='o')
plt.scatter(embedded_data[np.squeeze(test_labels ==4), 0], embedded_data[np.squeeze(test_labels ==4), 1], label=u'工况2异常样本', c = 'tomato', marker='o')
plt.scatter(embedded_data[np.squeeze(test_labels ==5), 0], embedded_data[np.squeeze(test_labels ==5), 1], label=u'工况3异常样本', c = 'sandybrown', marker='o')
