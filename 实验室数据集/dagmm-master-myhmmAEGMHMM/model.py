...
class DaGMM(nn.Module):
    ...
    def __init__(self, n_gmm = 4, latent_dim=7,num_states=6): #latent_dim为原始特征数，num_states为状态数
        ...
        self.layer2 = nn.Sequential(
            nn.Conv1d(3, 6, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm1d(6),
            nn.ReLU(),
            nn.MaxPool1d(2))
        ...
        self.layer4 = nn.Sequential(
            nn.Conv1d(12, 12, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm1d(12),
            nn.ReLU(),
            nn.MaxPool1d(2))

        self.Linear1 = nn.Linear(2400, 150)#3*window/2;  window = window/2/2/2 (192=3*128/2) #600
        self.Linear2 = nn.Linear(150, 100)
        self.Linear3 = nn.Linear(100, 50)
        self.Linear4 = nn.Linear(50, 30)
        self.Linear5 = nn.Linear(30, 10)
        self.Linear6 = nn.Linear(10, latent_dim-2)

        self.fc1 = nn.Linear(latent_dim-2, 10)  # map z_c to a higher dimensional space
        self.fc2 = nn.Linear(10, 30)
        self.fc3 = nn.Linear(30, 50)
        self.fc4 = nn.Linear(50, 100)
        self.fc5 = nn.Linear(100, 150)
        self.fc6 = nn.Linear(150, 2400)
        self.layer1_d = nn.Sequential(
            nn.ConvTranspose1d(12, 12, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm1d(12),
            nn.ReLU())
        ...
        self.layer3_d = nn.Sequential(
            nn.ConvTranspose1d(6, 3, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm1d(3),
            nn.ReLU())
        ...
        layers += [nn.Linear(10, num_states ** 2)]
        ...
        layers += [nn.Tanh()]
        layers += [nn.Dropout(p=0.5)]
        layers += [nn.Linear(10, num_states * n_gmm)]
        layers += [nn.Softmax(dim=1)]
    ...
    def forward(self, x):
        ...
        rec_cosine = F.cosine_similarity(x, dec, dim=1)#余弦相似性
        rec_euclidean = self.relative_euclidean_distance(x, dec)#相对欧几里得距离
        #unsqueeze(-1)表示在最后一个维度（最内层维度）上增加一个维度
        z = torch.cat([enc, rec_euclidean.unsqueeze(-1), rec_cosine.unsqueeze(-1)], dim=1)#拼接
