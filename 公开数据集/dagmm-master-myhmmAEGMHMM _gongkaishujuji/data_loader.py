...
class KDD99Loader(object):
    def __init__(self, data_path, mode="train"):
        ...
        # data_pin_abnormal_933 = np.loadtxt('D:/数据_刘嘉帅/combined_data_abnormal_aegmhmm_250.txt', dtype=str, delimiter=',')
        # data_pin_abnormal_25t = np.loadtxt('D:/数据_刘嘉帅/combined_data_abnormal_aegmhmm_0.25t.txt', dtype=str,
        #                                    delimiter=',')
        # data_pin_abnormal_sin = np.loadtxt('D:/数据_刘嘉帅/combined_data_abnormal_aegmhmm_sin.txt', dtype=str, delimiter=',')
        # data_pin_abnormal_zhengtai = np.loadtxt('D:/数据_刘嘉帅/combined_data_abnormal_aegmhmm_zhengtai.txt', dtype=str,
        #                                         delimiter=',')
        data_pin_normal = np.loadtxt('D:/变工况数据集/实验用/正常样本/normal_speed.txt', dtype=str, delimiter=',')
        data_pin_abnormal_pitting_10 = np.loadtxt('D:/变工况数据集/实验用/异常样本/abnormal_pitting.txt', dtype=str,
                                                  delimiter=',')
        data_pin_abnormal_break_10 = np.loadtxt('D:/变工况数据集/实验用/异常样本/abnormal_break.txt', dtype=str,
                                                delimiter=',')
        data_pin_abnormal_crack_10 = np.loadtxt('D:/变工况数据集/实验用/异常样本/abnormal_crack.txt', dtype=str,
                                                delimiter=',')
        data_pin_abnormal_wear_10 = np.loadtxt('D:/变工况数据集/实验用/异常样本/abnormal_wear.txt', dtype=str,
                                               delimiter=',')
        ...
        data_pin_normal = np.delete(data_pin_normal, np.array([2, 3, 4, 5, 6, 7]), 1)
        data_pin_abnormal_pitting_10 = np.delete(data_pin_abnormal_pitting_10, np.array([2, 3, 4, 5, 6, 7]), 1)
        data_pin_abnormal_break_10 = np.delete(data_pin_abnormal_break_10, np.array([2, 3, 4, 5, 6, 7]), 1)
        data_pin_abnormal_crack_10 = np.delete(data_pin_abnormal_crack_10, np.array([2, 3, 4, 5, 6, 7]), 1)
        data_pin_abnormal_wear_10 = np.delete(data_pin_abnormal_wear_10, np.array([2, 3, 4, 5, 6, 7]), 1)
        ...
        matrix_addfault = data_pin_abnormal_pitting_10
        # matrix_addfault = data_pin_abnormal_break_10
        # matrix_addfault = data_pin_abnormal_crack_10
        # matrix_addfault = data_pin_abnormal_wear_10
        ...
        matrix_nofault1 = np.reshape(matrix_nofault, (4, _all_length, -1))
        matrix_nofault2_train = matrix_nofault1[:, :_all_length // 2, :]  # (4,60000,2)
        ...
        matrix_nofault5_train = np.transpose(matrix_nofault4_train, (1, 2, 0))  # (150,2,1600)
        matrix_nofault5_test = np.transpose(matrix_nofault4_test, (1, 2, 0))  #

        matrix_addfault1 = np.reshape(matrix_addfault, (slid_window, 192000 // slid_window, -1))
        matrix_addfault2 = np.transpose(matrix_addfault1, (1, 2, 0))
        matrix_test = np.vstack((matrix_nofault5_test, matrix_addfault2))
        train_labels = np.vstack((np.zeros((150, 1))))
        test_labels = np.vstack((np.zeros((150, 1)), np.ones((120, 1))))

        self.train = matrix_nofault5_train
        self.train = self.train.reshape(-1, 3200)
        self.train_labels = train_labels

        self.test = matrix_test
        self.test_labels = test_labels
        self.test = self.test.reshape(-1, 3200)
