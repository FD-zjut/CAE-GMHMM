...
class Solver(object):
    ...
    def train(self):
        ...
        for e in range(start, self.num_epochs):
            ...
            for i, (input_data, labels) in enumerate(tqdm(self.data_loader)):
                ...
                loss['sample_energy'] = sample_energy.item() * self.lambda_energy
    ...
    def test(self):
        ...
        for it, (input_data, labels) in enumerate(self.data_loader):
            ...
            mu_sum += mu * batch_gamma_sum.unsqueeze(-1)  # keep sums of the numerator only仅保留分子的和
            cov_sum += cov * batch_gamma_sum.unsqueeze(-1).unsqueeze(-1)  # keep sums of the numerator only仅保留分子的和
            print("it:", it)  # 0-9
            N += input_data.size(0)
        ...
        for it, (input_data, labels) in enumerate(self.data_loader):
            ...
            # input_data = input_data.reshape(-1,1,4800)
            enc, dec, z, gamma = self.dagmm(input_data)
            sample_energy, cov_diag = self.dagmm.compute_energy(z, train_phi, train_mu, train_cov,
                                                                size_average=False)  # False表示不求和
        ...
        test_energy = np.concatenate(test_energy,axis=0)#(150,)
        test_z = np.concatenate(test_z,axis=0)
        test_labels = np.concatenate(test_labels,axis=0)

        # # 指定保存的文件路径
        file_path = 'D:/cw/dagmm-master-myhmm1221CAEGMMmulti_condi/test_z.txt'
        # 使用savetxt函数保存数据到txt文件
        ...
        thresh_real = np.percentile(train_energy, 100 - 33.3)#20 #合并能量中（训练集+测试集合）有50%为正常样本，所以取小于50%的为阈值
        # thresh = np.percentile(train_likehood_cw, 5) #表示有x%的数据小于或等于计算得到的thresh 值 #19.26
        # thresh_real = max(train_energy)
        # print("test_energy:", test_energy.shape, test_energy)
        print("Threshold :", thresh_real)
        # 根据阈值对测试能量进行二分类预测，生成预测值和真实值
        # pred = (test_energy > thresh).astype(int)# 如果测试能量大于阈值，则预测值为1，否则为0
        pred = (test_energy > thresh_real).astype(int)  # 如果测试能量大于阈值，则预测值为1，否则为0
        print("pred:", pred)
        gt = test_labels.astype(int)  # 真实的分类标签
        print("gt:", gt)
        ...
        print("Accuracy : {:0.4f}, Precision : {:0.4f}, Recall : {:0.4f}, F-score : {:0.4f}".format(accuracy, precision,
                                                                                                    recall, f_score))
        # 用max:Accuracy : 0.7867, Precision : 0.7698, Recall : 0.9700, F-score : 0.8584
        # 用百分比：Accuracy : 0.7733, Precision : 0.8235, Recall : 0.8400, F-score : 0.8317
