...
class Solver(object):
    ...
    def test(self):
        ...
        thresh = np.percentile(train_likehood_cw, 1) #表示有x%的数据小于或等于计算得到的thresh 值 #19.26
        ...
        # # 指定保存的文件路径
        file_path = 'D:/cw/dagmm-master-myhmm1221multi_cond - 副本/combined_likehood_cw.txt'
        # 使用savetxt函数保存数据到txt文件
        np.savetxt(file_path, combined_likehood_cw)

        # # 指定保存的文件路径
        file_path = 'D:/cw/dagmm-master-myhmm1221multi_cond - 副本/test_z.txt'
        # 使用savetxt函数保存数据到txt文件
        ...
        # plt.scatter(pca_x[:, 0], pca_x[:, 1], label='Principal Components', marker='^')
        # plt.legend()
        # plt.title('PCA Example')
        # plt.xlabel('Feature 1')
        # plt.ylabel('Feature 2')
        # plt.show()
        # np.save(r"D:\cw\dagmm-master-myhmm1221multi_cond - 副本\pca.npy", pca_x)
