...
class DaGMM(nn.Module):
    ...
    def __init__(self, n_gmm = 4, latent_dim=7,num_states=6): #latent_dim为原始特征数，num_states为状态数
        ...
        self.layer2 = nn.Sequential(
            nn.Conv1d(2, 12, kernel_size=3, stride=1, padding=1),
            #归一化层
            nn.BatchNorm1d(12),
            #激活函数
            nn.ReLU(),
        ...
        #特征提取和降维
        self.Linear1 = nn.Linear(4800, 150)#6*window/2;  window = window/2/2/2 (192=3*128/2) #600 6*1600/2
        ...
        self.layer3_d = nn.Sequential(
            nn.ConvTranspose1d(12, 2, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm1d(2),
            nn.ReLU())
    ...
    def forward(self, x):
        ...
        D1wei, D2wei = z.size()  # z(5,7) 3是拼接. D1wei是时间，D2wei是latent_dim
